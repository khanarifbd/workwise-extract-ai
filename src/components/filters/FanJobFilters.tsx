...
export const FanJobFilters = ({ 
  ...
  useEffect(() => {
    const stored = localStorage.getItem(`${STORAGE_KEY}-fan`);
    if (stored) {
      try {
  ...
  const savePresets = (newPresets: FilterPreset[]) => {
    ...
    localStorage.setItem(`${STORAGE_KEY}-fan`, JSON.stringify(newPresets));
  };
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input
            placeholder="Search jobs, names, phone..."
      ...
      {showAdvanced && (
        <div className="flex flex-wrap items-center gap-2 p-3 bg-muted/30 rounded-lg border border-border">
          ...
          {/* Status Filter - Fan specific statuses */}
