      ...
      <div className="flex-1 flex min-h-0 p-3 gap-3">
        ...
        <aside className="w-[420px] flex flex-col min-h-0 rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
          <Tabs value={activeStream} onValueChange={(v) => setActiveStream(v as ProgStream)} className="flex flex-col flex-1 min-h-0">
            ...
            <TabsContent value="dm" className="flex-1 min-h-0 mt-0 data-[state=inactive]:hidden flex flex-col">
              <StreamColumn
            ...
            <TabsContent value="aa" className="flex-1 min-h-0 mt-0 data-[state=inactive]:hidden flex flex-col">
              <StreamColumn
                    ...
                    {weeks.map(({ weekKey, weekLabel, days }) => {
                      ...
                      return (
                        <Collapsible key={weekKey} open={wOpen} onOpenChange={() => toggleWeek(weekKey)}>
                          <CollapsibleTrigger asChild>
                            <button className="w-full flex items-center gap-2 px-2 py-1.5 rounded bg-sky-100 hover:bg-sky-200/70 border border-sky-200 transition-colors">
                              <ChevronDown className={cn("h-3.5 w-3.5 shrink-0 transition-transform text-sky-600 dark:text-sky-300", !wOpen && "-rotate-90")} />
                              <span className="text-[11px] font-bold uppercase tracking-wider text-sky-700 dark:text-sky-200">Week · {weekLabel}</span>
                              <span className="ml-auto text-[10px] font-semibold text-sky-600/80 dark:text-sky-300/80">{weekCount}</span>
                            </button>
                          </CollapsibleTrigger>
                          <CollapsibleContent>
